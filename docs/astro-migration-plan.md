...
プランどおり8フェーズを実施した。プランから変更した点は以下のとおり。

| 項目 | プラン | 実際 | 理由 |
|---|---|---|---|
| MapLibre のバージョン | npm 化（版は未指定） | 6.10.0 | vendor 版と同じ 5 系を入れたところ、5.x 全体が critical の XSS 勧告 GHSA-jrc7-96c5-q579 の対象だった。修正版が 6.10.0 のみのためメジャーアップグレードし、動作を確認した |
| スコープ付きスタイル | 言及なし | `scopedStyleStrategy: "where"` を指定 | 既定の属性方式は詳細度を上げるため、`:lang(ja) p { word-break: auto-phrase }` に勝ってしまい、モバイルの日本語の折り返しが変わった |
| Markdown の改行 | 言及なし | `@astrojs/markdown-remark` ＋ `remark-breaks` を追加 | Astro 7 の既定プロセッサには改行オプションがなく、原稿の改行が消えて `①②③` などが1行につながった |
| Node のバージョン | 22 | 22.22.2 | nodenv が完全一致のバージョン指定を必要とするため |
| `/join/` の FAQ | FAQ を置く | 5問を新規作成 | **内容は既存サイトの記述（機材貸出あり／中学生以下は保護者同伴／約1.5kmを2時間／音源はアーカイブ掲載）から起こしたもので、公開前にクラブ側の確認が必要** |

### 検証結果

- フェーズ1（移植直後）：移行前サイトとのフルページ比較でデスクトップは md5 一致、モバイルは最大差 2/255・318ピクセル（アンチエイリアスの差）
- フェーズ2-3（分割後）：同じ比較でデスクトップはピクセル一致、モバイルの差はしきい値以下
- 全13ページのビルド、`astro check` 0 エラー、内部リンク・アセット参照の全解決、`npm audit` 0 件
- 390px 幅で横スクロールが発生しないことを確認

### 残っている作業

- `master` へのマージと、GitHub Actions での初回デプロイ確認
- 過去の着手ブランチ `astronization` / `codex/astro` のクローズ
- `/join/` の FAQ 文言の確認
